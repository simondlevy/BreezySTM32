...
static bool i2cHandleHardwareFailure(void)
{
    ...
    HardwareWire::init(I2Cx_index);
    return false;
}

bool HardwareWire::writeBuffer(uint8_t addr_, uint8_t reg_, uint8_t len_, uint8_t *data)
{
...
bool HardwareWire::write(uint8_t addr_, uint8_t reg_, uint8_t data)
{
    return HardwareWire::writeBuffer(addr_, reg_, 1, &data);
}

bool HardwareWire::read(uint8_t addr_, uint8_t reg_, uint8_t len, uint8_t *buf)
{
    uint32_t timeout = I2C_DEFAULT_TIMEOUT;
    ...
    if (SR1Register & 0x0700) {
        ...
        if (!(SR1Register & 0x0200) && !(I2Cx->CR1 & 0x0200)) {         // if we dont have an ARLO error, ensure sending of a stop
            if (I2Cx->CR1 & 0x0100) {                                   // We are currently trying to send a start, this is very bad as start, stop will hang the peripheral
                ...
                HardwareWire::init(I2Cx_index);                                    // reset and configure the hardware
            } else {
